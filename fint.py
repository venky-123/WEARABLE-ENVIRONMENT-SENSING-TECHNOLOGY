...
db = MySQLdb.connect("localhost","moderator","moderator","main_aat" )
...
format_str="""INSERT INTO root('journey_id','timestamp','cluster_id','gps_tags','image_tags') VALUES ('','{timestamp}','','','');"""
sql_command=foramt_str.format(timestamp=msg.timestamp)  #inserting time stamp into the values and updating the data base
...
while(1):
    ...
    ser= serial.Serial('/dev/ttyUSB0',9600)                #for serial communication of GPS data through USB
    if '$GPGGA' in s:                                      
		r=r+1
		if(r>2):
			msg=pynmea2.parse(s)                   #object for gps data
			print msg.lat                          #prints latitude in a particular format given belw(string data type)
        	        print msg.lon 
			...
			sql_command_2=format_str.format(j_id=j_id,loc_long=msg.lon,loc_lat=msg.lat,timestamp=msg.timestamp)
			...
			sql_command_3=format_str.format(j_id=j_id,time_stamp=msg.timestamp)
...
#Connection to SQL server!
db = MySQLdb.connect("localhost","moderator","moderator","root_aat" )
